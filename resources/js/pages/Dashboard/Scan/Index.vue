...
function navigateToToken(token: string): void {
    ...
    router.visit(`/dashboard/scan/${encodeURIComponent(raw)}`);
}
...
async function startScanner(): Promise<void> {
    ...
    try {
        ...
        if (/Permission|NotAllowedError/i.test(message)) {
            ...
            errorMessage.value = t('dashboard.scan.index.camera_denied');
        } else if (/NotFoundError|no.camera/i.test(message)) {
            cameraStatus.value = 'no-camera';
            errorMessage.value = t('dashboard.scan.index.no_camera');
        } else {
            cameraStatus.value = 'error';
            errorMessage.value = t('dashboard.scan.index.scanner_error');
        }
    }
}
...
<template>
    <Head :title="t('dashboard.scan.index.title')" />
                ...
                >
                    <div class="flex-1 space-y-2">
                        <Label for="manual-token">
                            {{ t('dashboard.scan.index.manual_link') }}
                        </Label>
                        <Input
                            id="manual-token"
                            v-model="manualToken"
                            ...
                            type="text"
                            autocomplete="off"
                            :placeholder="t('dashboard.scan.index.manual_link')"
                        />
                    </div>
                    <Button
                        type="submit"

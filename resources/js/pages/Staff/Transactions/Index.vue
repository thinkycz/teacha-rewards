...
function typeLabel(value: string): string {
    switch (value) {
        case 'purchase_cashback':
            return t('staff.transactions.index.type_purchase_cashback');
        case 'redeem':
            return t('staff.transactions.index.type_redeem');
        case 'manual_add':
            return t('staff.transactions.index.type_manual_add');
        case 'manual_subtract':
            return t('staff.transactions.index.type_manual_subtract');
        case 'manual_set':
            return t('staff.transactions.index.type_manual_set');
        default:
            return value;
    }
}
...
const typeOptions = computed(() => [
    { value: '', label: t('staff.transactions.index.type_all') },
    ...props.type_options.map((value) => ({ value, label: typeLabel(value) })),
]);
</script>

<template>
    <Head :title="t('staff.transactions.index.title')" />

    <StaffLayout :title="t('staff.transactions.index.title')">
        <div class="space-y-5">
            <header>
                <h1 class="text-2xl font-semibold text-charcoal-900">
                    {{ t('staff.transactions.index.heading') }}
                </h1>
            </header>
                ...
                >
                    <div class="relative">
                        ...
                        <Input
                            ...
                            type="search"
                            name="q"
                            :placeholder="t('staff.transactions.index.search_placeholder')"
                            class="pl-8"
                        />
                    </div>
                    <Select
            ...
            <section v-if="transactions.length === 0">
                ...
                >
                    {{ t('staff.transactions.index.empty') }}
                </div>
            </section>
            ...
            <ul v-else class="space-y-2">
                ...
                >
                    <component
                        ...
                        :href="tx.wallet_id ? `/staff/wallets/${tx.wallet_id}` : undefined"
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <div class="min-w-0 flex-1">
                                ...
                                >
                                    {{ t('staff.transactions.index.purchase_amount') }}: {{ tx.purchase_amount }} Kč
                                </p>
                                <p
                                    v-if="tx.note"
                                    class="text-[10px] text-charcoal-500"
                                >
                                    {{ t('staff.transactions.index.note') }}: {{ tx.note }}
                                </p>
                                <p
                                    v-if="tx.staff_name"
                                    class="text-[10px] text-charcoal-500"
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-[10px] uppercase tracking-wider text-charcoal-500">
                                    {{ t('staff.transactions.index.balance_after') }}
                                </p>

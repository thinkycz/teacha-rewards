...
function submitPurchase(): void {
    purchaseForm.post(`/staff/wallets/${props.wallet.id}/purchase`, {
        preserveScroll: true,
        onSuccess: () => {
            purchaseForm.reset();
...
function submitRedeem(): void {
    redeemForm.post(`/staff/wallets/${props.wallet.id}/redeem`, {
        preserveScroll: true,
        onSuccess: () => {
            redeemForm.reset();
            openAction.value = null;
...
function submitAdjust(): void {
    adjustForm.post(`/staff/wallets/${props.wallet.id}/adjust`, {
        preserveScroll: true,
        onSuccess: () => {
            adjustForm.reset();
            openAction.value = null;
...
async function toggleStatus(): Promise<void> {
    if (isActive.value) {
        const ok = await confirmDialog.confirm(t('staff.wallets.show.disable_confirm'), {
            variant: 'danger',
            confirmLabel: t('staff.wallets.show.disable'),
        });
        if (!ok) {
            return;
        }
        router.post(`/staff/wallets/${props.wallet.id}/disable`, {}, { preserveScroll: true });
    } else {
        const ok = await confirmDialog.confirm(t('staff.wallets.show.enable_confirm'), {
            confirmLabel: t('staff.wallets.show.enable'),
        });
        if (!ok) {
            return;
        }
        router.post(`/staff/wallets/${props.wallet.id}/enable`, {}, { preserveScroll: true });
    }
}

function formatDateTime(value: string | null): string {
    if (value === null) {
        return t('staff.wallets.show.never_used');
    }
...
<template>
    <Head :title="t('staff.wallets.show.title', { name: wallet.first_name })" />

    <StaffLayout :title="t('staff.wallets.show.title', { name: wallet.first_name })">
        <div class="space-y-6">
            <section
                v-if="!isActive"
            ...
            >
                <div class="flex items-start gap-2">
                    ...
                    <p>{{ t('staff.wallets.show.disabled_notice') }}</p>
                </div>
            </section>
            ...
            <!-- Customer metadata -->
            <section
                class="grid grid-cols-2 gap-3 rounded-2xl border border-outline-glass bg-white p-4 text-xs shadow-sm sm:grid-cols-3"
            >
                <div>
                    <p class="font-semibold uppercase tracking-wider text-charcoal-500">
                        {{ t('staff.wallets.show.customer') }}
                    </p>
                ...
                <div>
                    <p class="font-semibold uppercase tracking-wider text-charcoal-500">
                        {{ t('staff.wallets.show.phone') }}
                    </p>
                    <p class="mt-1 font-mono text-sm text-charcoal-900">
                        {{ wallet.phone }}
                    </p>
                </div>
                <div>
                    <p class="font-semibold uppercase tracking-wider text-charcoal-500">
                        {{ t('staff.wallets.show.wallet_number') }}
                    </p>
                    <p class="mt-1 font-mono text-sm text-charcoal-900">
                ...
                <div>
                    <p class="font-semibold uppercase tracking-wider text-charcoal-500">
                        {{ t('staff.wallets.show.lifetime_earned') }}
                    </p>
                    <p class="mt-1 text-sm text-charcoal-900">
                ...
                <div>
                    <p class="font-semibold uppercase tracking-wider text-charcoal-500">
                        {{ t('staff.wallets.show.lifetime_redeemed') }}
                    </p>
                    <p class="mt-1 text-sm text-charcoal-900">
                ...
                <div>
                    <p class="font-semibold uppercase tracking-wider text-charcoal-500">
                        {{ t('staff.wallets.show.last_used') }}
                    </p>
                    <p class="mt-1 text-sm text-charcoal-900">
            ...
            <div class="flex justify-end">
                ...
                >
                    {{ t('staff.wallets.show.view_public') }}
            ...
            <section>
                <h2 class="mb-3 text-sm font-semibold uppercase tracking-wider text-charcoal-500">
                    {{ t('staff.wallets.show.actions_heading') }}
                </h2>
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <button
                        type="button"
                        :disabled="!isActive"
                        class="flex items-center justify-center gap-2 rounded-2xl border border-outline-glass bg-white px-3 py-3 text-xs font-semibold text-charcoal-900 shadow-sm transition hover:border-matcha-300 hover:bg-sage-50 disabled:cursor-not-allowed disabled:opacity-50"
                        :class="{ 'border-matcha-500 bg-sage-50': openAction === 'purchase' }"
                        @click="openPanel('purchase')"
                    >
                        <ShoppingBag :size="16" />
                        {{ t('staff.wallets.show.log_purchase') }}
                    </button>
                    <button
                        type="button"
                        :disabled="!isActive"
                    ...
                    >
                        <Sliders :size="16" />
                        {{ t('staff.wallets.show.manual_adjust') }}
                    </button>
                    <button
                        type="button"
                        class="flex items-center justify-center gap-2 rounded-2xl border border-outline-glass bg-white px-3 py-3 text-xs font-semibold text-charcoal-900 shadow-sm transition hover:border-amber-300 hover:bg-amber-50"
                        @click="toggleStatus"
                    >
                        <component
                            ...
                            :size="16"
                        />
                        {{ isActive ? t('staff.wallets.show.disable') : t('staff.wallets.show.enable') }}
                    </button>
                </div>
            </section>
                ...
                >
                    <div class="space-y-2">
                        ...
                        <Input
                            ...
                            step="0.01"
                            min="0.01"
                            :placeholder="t('staff.forms.purchase_amount')"
                        ...
                        <p class="text-[10px] text-charcoal-500">
                            {{ t('staff.forms.purchase_amount_help') }}
                        </p>
                ...
                >
                    <div class="space-y-2">
                        ...
                        <Input
                            ...
                            :placeholder="t('staff.forms.redeem_amount')"
                        ...
                        />
                        <p class="text-[10px] text-charcoal-500">
                            {{ t('staff.forms.redeem_amount_help') }}
                        </p>
                ...
                >
                    <div class="space-y-2">
                        <Label for="adjust_type" required>
                            {{ t('staff.forms.adjust_type') }}
                        </Label>
                        <Select
                            id="adjust_type"
                            v-model="adjustForm.type"
                            :options="[
                                { value: 'add', label: t('staff.forms.adjust_type_add') },
                                { value: 'subtract', label: t('staff.forms.adjust_type_subtract') },
                                { value: 'set', label: t('staff.forms.adjust_type_set') },
                            ]"
                    ...
                    <div class="space-y-2">
                        ...
                        <Input
                            ...
                            step="0.01"
                            min="0.01"
                            :placeholder="t('staff.forms.adjust_amount')"
                        ...
                        />
                        <p class="text-[10px] text-charcoal-500">
                            {{ t('staff.forms.adjust_amount_help') }}
                        </p>
                    ...
                    <div class="space-y-2">
                        <Label for="adjust_note" required>
                            {{ t('staff.forms.adjust_note') }}
                        </Label>
                        <Input
                            id="adjust_note"
                            v-model="adjustForm.note"
                            type="text"
                            :placeholder="t('staff.forms.adjust_note')"
                        ...
                        />
                        <p class="text-[10px] text-charcoal-500">
                            {{ t('staff.forms.adjust_note_help') }}
                        </p>

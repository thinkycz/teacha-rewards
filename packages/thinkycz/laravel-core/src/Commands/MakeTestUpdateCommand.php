...
class MakeTestUpdateCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:test-update';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD update controller test for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/test.update.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('UpdateControllerTest')->value();
    ...
    {
        if (Str::endsWith($name, 'UpdateControllerTest')) {
            $name = Str::before($name, 'UpdateControllerTest');
        }

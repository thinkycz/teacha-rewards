...
class MakeCrudDestroyCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:crud-destroy';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD destroy controller for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/controller.destroy.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('DestroyController')->value();
    ...
    {
        if (Str::endsWith($name, 'DestroyController')) {
            $name = Str::before($name, 'DestroyController');
        }

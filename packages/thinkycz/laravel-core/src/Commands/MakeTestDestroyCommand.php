...
class MakeTestDestroyCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:test-destroy';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD destroy controller test for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/test.destroy.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('DestroyControllerTest')->value();
    ...
    {
        if (Str::endsWith($name, 'DestroyControllerTest')) {
            $name = Str::before($name, 'DestroyControllerTest');
        }

...
class MakeTestShowCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:test-show';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD show controller test for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/test.show.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('ShowControllerTest')->value();
    ...
    {
        if (Str::endsWith($name, 'ShowControllerTest')) {
            $name = Str::before($name, 'ShowControllerTest');
        }

...
class MakeCrudIndexCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:crud-index';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD index controller for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/controller.index.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('IndexController')->value();
    ...
    {
        if (Str::endsWith($name, 'IndexController')) {
            $name = Str::before($name, 'IndexController');
        }

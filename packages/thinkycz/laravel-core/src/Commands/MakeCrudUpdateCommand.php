...
class MakeCrudUpdateCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:crud-update';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD update controller for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/controller.update.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('UpdateController')->value();
    ...
    {
        if (Str::endsWith($name, 'UpdateController')) {
            $name = Str::before($name, 'UpdateController');
        }

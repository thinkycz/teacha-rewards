...
class MakeTestStoreCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:test-store';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD store controller test for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/test.store.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('StoreControllerTest')->value();
    ...
    {
        if (Str::endsWith($name, 'StoreControllerTest')) {
            $name = Str::before($name, 'StoreControllerTest');
        }

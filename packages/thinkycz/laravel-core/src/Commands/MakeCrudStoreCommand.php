...
class MakeCrudStoreCommand extends GeneratorCommand
{
    /**
     * @inheritDoc
     */
    protected $name = 'make:crud-store';

    /**
     * @inheritDoc
     */
    protected $description = 'Create CRUD store controller for a given model';
    ...
    {
        return $this->resolveStubPath('stubs/controller.store.stub');
    }
    ...
    {
        $name = (new Stringable($name))->replaceFirst($this->rootNamespace(), '')->finish('StoreController')->value();
    ...
    {
        if (Str::endsWith($name, 'StoreController')) {
            $name = Str::before($name, 'StoreController');
        }

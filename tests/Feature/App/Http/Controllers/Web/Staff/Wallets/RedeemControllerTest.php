...
\test('POST /staff/wallets/{wallet}/redeem debits the wallet balance', function (): void {
    ...
    $response = $this->actingAs($staff)->post(
        "/staff/wallets/{$wallet->getKey()}/redeem",
        ['amount' => '20.00'],
        $this->inertiaHeaders(),
    );
...
\test('POST /staff/wallets/{wallet}/redeem rejects an amount larger than the balance with a friendly error', function (): void {
    $staff = User::factory()->staff()->create();
    $wallet = RewardWallet::factory()->create([
...
\test('POST /staff/wallets/{wallet}/redeem rejects a non-positive amount', function (): void {
    $staff = User::factory()->staff()->create();
    $wallet = RewardWallet::factory()->create([
        'rewards_balance' => '50.00',
    ]);
...
\test('POST /staff/wallets/{wallet}/redeem is forbidden to a guest', function (): void {
    $wallet = RewardWallet::factory()->create([
        'rewards_balance' => '50.00',
    ]);
